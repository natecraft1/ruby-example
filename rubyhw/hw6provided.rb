...
class Piece 
  ...
  def initialize (point_array, board)
    ...
    puts "#{self}@board: #{@board}"
    puts "#{self}@all_rotations: #{@all_rotations}"
    puts "#{self}@rotation_index: #{@rotation_index}"
    puts "#{self}@color: #{@color}"
    puts "#{self}@base_position: #{@base_position}"
    puts "#{self}@moved: #{@moved}"
    puts "========================="
  end

  def current_rotation
    puts "#{self} current_rotation called"
    @all_rotations[@rotation_index]
  end
  
  def moved
    puts "#{self} moved called"
    @moved
  end

  def position
    puts "#{self} position called"
    @base_position
  end

  def color
    puts "#{self} color called"
    @color
  end

  def drop_by_one
    puts "#{self} drop_by_one called"
    @moved = move(0, 1, 0)
  end
  ...
  def move (delta_x, delta_y, delta_rotation)
    ...
    puts "#{self}move:potential: #{potential}"
    ...
    puts "#{self}@base_position: #{@base_position}, rotation_index: #{@rotation_index}"
    puts "#{self}move:moved: #{moved}"
    moved
  end
  ...
  def self.next_piece (board)
    puts "#{self}self.next_piece called with (#{board})"
...
class Board
  ...
  def initialize (game)
    ...
    @game = game
    @delay = 500
    puts "====== initialize Board with (#{game}) ======"
    puts "#{self}@grid: #{@grid}"
    puts "#{self}@current_block: #{@current_block}"
    puts "#{self}@score: #{@score}"
    puts "#{self}@game: #{@game}"
    puts "#{self}@delay: #{@delay}"
  end
  ...
  def run
    puts "#{self} run called"
    ran = @current_block.drop_by_one
    puts "#{self}run:ran: #{ran}"
    if !ran
      store_current
      if !game_over?
        next_piece
  ...
  puts "#{self} rotate_clockwise called"
  ...
  def rotate_counter_clockwise
    puts "#{self} rotate_counter_clockwise called"
    if !game_over? and @game.is_running?
      @current_block.move(0, 0, -1)
    end
    draw
  ...
  def drop_all_the_way
    puts "#{self} drop_all_the_way called"
    if @game.is_running?
      ran = @current_block.drop_by_one
      while ran
  ...
  def store_current
    puts "#{self} store_current called"
  ...
  # currently empty.
  def empty_at (point)
    puts "#{self} empty_at called with #{point}"
  ...
  def remove_filled
    puts "#{self} remove_filled called"
...
class Tetris
  ...
  def initialize
    ...
    buttons
    run_game
    puts "====== initialize Tetris ======"
    puts "#{self}@root: #{@root}"
    puts "#{self}@timer: #{@timer}"
    puts "#{self}@running: #{@running}"
  end

  # creates a canvas and the board that interacts with it
  def set_board
    puts "#{self} set_board called"
    @canvas = TetrisCanvas.new
    @board = Board.new(self)
    @canvas.place(@board.block_size * @board.num_rows + 3,
                  @board.block_size * @board.num_columns + 6, 24, 80)
    @board.draw
    puts "#{self}@canvas: #{@canvas}"
    puts "#{self}@board: #{@board} called with (#{self})"
  end

  def key_bindings  
    puts "#{self} key_bindings called"
  ...
  def draw_piece (piece, old=nil)
    puts "#{self}: draw_piece called with (piece: #{piece}, old: #{old} )"
    ...
    blocks.map{|block| 
    puts "size: #{size}, start: #{start} block: #{block}"
    puts "x: #{start[0]*size + block[0]*size + 3}"
    puts "y: #{start[1]*size + block[1]*size}"
    puts "x1: #{start[0]*size + size + block[0]*size + 3}"
    puts "y1: #{start[1]*size + size + block[1]*size}"
